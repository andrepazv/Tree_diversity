...
write_csv(comm %>% mutate(accepted_bin = gsub(" ", "_", accepted_bin)), "cleaned_data/REV_Community_matrix.csv")
write_csv(dt_mat %>% mutate(accepted_bin = gsub(" ", "_", accepted_bin)), "cleaned_data/REV_Trait_matrix.csv")
write.tree(pruned_tree, "cleaned_data/REV_Pruned_tree.csv")
